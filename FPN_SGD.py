...
def main():
    ...
    writer = SummaryWriter('./keep/sgd_0.001')
    ...
    optimizer = torch.optim.SGD([
       ...
       # {'params': model.module.classifier.parameters(), 'lr': args.lr * 10}
                                    ], args.lr, momentum=args.momentum, weight_decay=args.weight_decay) #SGD
...
def save_checkpoint(state, is_best, filename='checkpoint_sgd.pth.tar'):
    torch.save(state, filename)
    if is_best:
        shutil.copyfile(filename, 'model_best_sgd.pth.tar')
